...
Button.Image = "rbxassetid://85105976391315"
...
-- UI REPLACEMENT (SAFE)
-- =========================
-- NOTE: The following block replaces the previous window with a modern Fluent-style UI.
-- It creates toggles/checkboxes but DOES NOT implement or enable any ESP/aiming/cheat features
-- in live games. For educational purposes, a commented "Demo ESP" module is included below that
-- will only work in Roblox Studio if you create a Workspace.DemoObjects folder. Do NOT enable
-- this in published games or use it to gain unfair advantage.

-- Safely remove existing Window if present
pcall(function()
    if Window and type(Window) == "table" and Window.Close then
        pcall(function() Window:Close() end)
    end
end)

-- Create a fresh Fluent-like window (visual only)
local ok, Fluent = pcall(function()
    return loadstring(game:HttpGet("https://github.com/dawid-scripts/Fluent/releases/latest/download/main.lua"))()
end)
if not ok or not Fluent then
    -- fallback: create a minimal UI using Roblox instances (Fluent library not available)
    local Players = game:GetService("Players")
    local player = Players.LocalPlayer
    local playerGui = player:WaitForChild("PlayerGui")
    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "TenHub_FluentFallback"
    ...
    title.Text = "Ten Hub — Fluent UI (Fallback)"
else
    -- Create a modern window using Fluent
    Window = Fluent:CreateWindow({
        Title = "Ten Hub",
        SubTitle = "Blox Fruits - UI Replaced",
        TabWidth = 160,
        Theme = "Darker",
        Acrylic = true,
    ...
    -- Add stylish toggles to Visual tab (no ESP behavior here)
    local visualTab = Tabs2.Visual
    visualTab:AddToggle({ Text = "Show Enemy Box (Demo Only)", Flag = "Demo_ShowEnemyBox", Default = false, Tooltip = "Visual toggle only — demo mode" })
    visualTab:AddToggle({ Text = "Show Fruit Count (Demo Only)", Flag = "Demo_ShowFruitCount", Default = true, Tooltip = "Visual toggle only — demo mode" })
    visualTab:AddToggle({ Text = "Show Health Bars (Demo Only)", Flag = "Demo_ShowHealth", Default = true, Tooltip = "Visual toggle only — demo mode" })

    -- A little info panel
    Tabs2.Info:AddLabel({ Text = "UI replaced with modern Fluent-style interface." })
    Tabs2.Info:AddLabel({ Text = "Important: Demo ESP (studio-only) is commented below. Do NOT use in real games." })
end

-- =========================
-- DEMO ESP MODULE (COMMENTED / STUDIO ONLY)
-- =========================
--[[ 
-- The following 'DemoESP' module is provided only for local testing inside ROBLOX STUDIO.
-- It will draw BillboardGuis and basic info for parts placed under Workspace.DemoObjects.
-- This module is commented out by default to avoid accidental use. To enable in Studio:
-- 1) Create a Folder named "DemoObjects" under Workspace.
-- 2) Place some Parts named "DemoEnemy_*" and "DemoFruit_*" under that folder.
-- 3) Uncomment the code below (remove the surrounding --[[ and ]]) and run in Studio only.

local RunService = game:GetService("RunService")
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
local demoFolder = workspace:FindFirstChild("DemoObjects")
if demoFolder then
    local function createBillboardFor(part)
        if part:FindFirstChild("DemoBillboard") then return end
        local bb = Instance.new("BillboardGui")
        bb.Name = "DemoBillboard"
        bb.Adornee = part
        bb.Size = UDim2.new(0,140,0,54)
        bb.AlwaysOnTop = true
        ...
        nameLabel.Size = UDim2.new(0.6, -6, 0, 20)
        ...
        distLabel.Size = UDim2.new(0.38, -6, 0, 20)
        ...
        healthFrame.Size = UDim2.new(0.92, 0, 0, 8)
        healthFrame.Position = UDim2.new(0.04, 0, 0, 30)
        healthFrame.BackgroundTransparency = 0.6
        healthFrame.BorderSizePixel = 0
        healthFrame.BackgroundColor3 = Color3.fromRGB(40,40,44)

        local hpFill = Instance.new("Frame", healthFrame)
        hpFill.Size = UDim2.new(1,0,1,0)
        hpFill.Position = UDim2.new(0,0,0,0)
        hpFill.BorderSizePixel = 0
        hpFill.BackgroundColor3 = Color3.fromRGB(100,220,130)
        Instance.new("UICorner", hpFill).CornerRadius = UDim.new(0,6)

        part:SetAttribute("DemoHealth", part:GetAttribute("DemoHealth") or math.random(20,100))
        part:SetAttribute("DemoMaxHealth", 100)
        ...
        RunService.RenderStepped:Connect(function()
            ...
            local h = part:GetAttribute("DemoHealth") or 0
            local m = part:GetAttribute("DemoMaxHealth") or 100
            local perc = math.clamp(h/m, 0, 1)
            hpFill.Size = UDim2.new(perc,0,1,0)
        end)
    end

    for _, p in ipairs(demoFolder:GetChildren()) do
        if p:IsA("BasePart") then
            createBillboardFor(p)
        end
    end
end
-- End of DemoESP (commented)
]]

-- End of appended safe UI + demo block
